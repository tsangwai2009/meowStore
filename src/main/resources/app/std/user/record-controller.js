...
{ 
	...
	app.controller('RecordController', ['$scope', 'buyService', RecordController]);
	
	function RecordController($scope, buyService)
	{
		$scope.check = function(){
			buyService.checkLogin().then(function(result)
			{
				if(result.data[0]["authority"]=="ROLE_USER"){
					...
					$scope.checkBalance();
					buyService.findCashAll().then(function(result)
					{
						$scope.cashData = result.data;
						if(result.data) result.data.forEach(record => {
					...
					buyService.findChequeAll().then(function(result)
					{
